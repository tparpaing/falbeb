...
function calculateFaceMove(e) {
    ...
    if ('not-happy' in mouth.classList) {
        mouthR += Math.cos(180);
    }
    chinX = mouthX * 0.8;
    chinY = mouthY * 0.5;
...
let messageDiv = document.querySelector('#formStatusMessage');
if (messageDiv) {
    let errorEmail = document.querySelector('.inputGroup1 .error');
    let errors = {};
    errors['email'] = false;
    errors['pwd'] = false;
    ...
    if (errorEmail) {
        email.addEventListener('change', (event) => {
            let errorEmail = document.querySelector('.inputGroup1 .error');
            let successEmail = document.querySelector('.inputGroup1 .success');
            ...
            if (email.value.match(re) !== null) {
                ...
                if (!successEmail) {
                    ...
                    const textnode = document.createTextNode("L'adresse e-mail renseignée est correcte");
                    node.appendChild(textnode);
                    node.className = 'success';
                    document.querySelector('.inputGroup1').appendChild(node);
                }
            } else {
                errors['email'] = true;
                if (successEmail) { successEmail.remove(); }
                if (!errorEmail) {
                    const node = document.createElement("p");
                    const textnode = document.createTextNode("L'adresse e-mail renseignée est invalide");
                    node.appendChild(textnode);
                    node.className = 'error';
                    document.querySelector('.inputGroup2').appendChild(node);
                }
            }

            setFormStatusMessage(errors, messageDiv);
        });
    }

    password.addEventListener('input', (event) => {
        let errorPassword = document.querySelector('.inputGroup2 .error');
        let successPassword = document.querySelector('.inputGroup2 .success');
        let len = 8;
        let reNumber = new RegExp('[0-9]');
        let reLowercase = new RegExp('[a-z]');
        let reUppercase = new RegExp('[A-Z]');
        let reSpecialchars = new RegExp('[^a-zA-Z0-9_]');
        let verif = {};
        verif['len'] = (password.value.length >= len);
        verif['number'] = (password.value.match(reNumber) !== null);
        verif['lowercase'] = (password.value.match(reLowercase) !== null);
        verif['uppercase'] = (password.value.match(reUppercase) !== null);
        verif['specialChars'] = (password.value.match(reSpecialchars) !== null);
        let messageArr = [
            verif['len'] ? '' : ' 8 caractères',
            verif['number'] ? '' : ' un nombre',
            verif['uppercase'] ? '' : ' une lettre majuscule',
            verif['lowercase'] ? '' : ' une lettre minuscule',
            verif['specialChars'] ? '' : ' un caractère spécial'
        ];
        let message = messageArr.reduce((acc, item, index) => {
            if (item.length !== 0) {
                return acc + (item + ',');
            }
            return acc;
        ...
        if (message === 'Le mot de passe doit contenir au moin') {
            errors['pwd'] = false;
            if (errorPassword) { errorPassword.remove(); }
            if (!successPassword) {
                const node = document.createElement("p");
                const textnode = document.createTextNode("Le mot de passe renseigné est correct");
                node.appendChild(textnode);
                node.className = 'success';
                document.querySelector('.inputGroup2').appendChild(node);
            }
            ...
            errors['pwd'] = true;
            if (successPassword) { successPassword.remove(); }
            if (!errorPassword) {
                const node = document.createElement("p");
...
function setFormStatusMessage(errors, messageDiv) {
    let formStatusCircle = document.querySelector('#formStatusCircle');
    if (!errors['email'] && !errors['pwd']) {
        messageDiv.querySelector('.text').innerHTML = "Il n'y a plus d'erreur dans le formulaire !";
        messageDiv.className = 'messageIntoForm success';
        if (formStatusCircle) {
            formStatusCircle.style.fill = 'rgba(0, 148, 50, .5)';
        }
    } else {
        messageDiv.querySelector('.text').innerHTML = "Il y des erreurs dans le formulaire";
        messageDiv.className = 'messageIntoForm error';
        if (formStatusCircle) {
            formStatusCircle.style.fill = 'rgba(235, 47, 6, .5)';
        }
    }
}

document.querySelector('i.help').innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M256 0C114.6 0 0 114.6 0 256s114.6 256 256 256s256-114.6 256-256S397.4 0 256 0zM256 464c-114.7 0-208-93.31-208-208S141.3 48 256 48s208 93.31 208 208S370.7 464 256 464zM256 336c-18 0-32 14-32 32s13.1 32 32 32c17.1 0 32-14 32-32S273.1 336 256 336zM289.1 128h-51.1C199 128 168 159 168 198c0 13 11 24 24 24s24-11 24-24C216 186 225.1 176 237.1 176h51.1C301.1 176 312 186 312 198c0 8-4 14.1-11 18.1L244 251C236 256 232 264 232 272V288c0 13 11 24 24 24S280 301 280 288V286l45.1-28c21-13 34-36 34-60C360 159 329 128 289.1 128z"/></svg>';

let tooltips = document.querySelectorAll('i[data-tooltip]');
tooltips.forEach((elem) => {
    let span = document.createElement("span");
    let textNode = document.createTextNode(elem.getAttribute('data-tooltip'));
    ...
    if (navigator.userAgent.toLowerCase().match(/mobile/i)) {
        elem.addEventListener('click', (event) => {
            ...
            if (elem.className.match('tooltip-active') !== null) {
                elem.className = elem.className.replace('tooltip-active', '');
            } else {
                elem.className += ' tooltip-active';
            }
        });
    }
});