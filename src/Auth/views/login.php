...
<form action="<?= $router->generateUri('auth.login.post') ?>" method="POST">
    <div class="homepage">
        <a href="<?= $router->generateUri('index') ?>">
    ...
    <div class="svgContainer">
        <div>
            <svg class="mySVG" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200">
                ...
                <?php if ($success) : ?>
                    <circle id="formStatusCircle" cx="100" cy="100" r="100" fill="rgba(0, 148, 50, .5)" />
                <?php elseif (!empty($errors)) : ?>
                    <circle id="formStatusCircle" cx="100" cy="100" r="100" fill="rgba(235, 47, 6, .5)" />
                <?php endif; ?>
            </svg>
        </div>
    </div>
    ...
    <div class="inputGroup inputGroup2">
        <label for="loginPassword" id="loginPasswordLabel">Mot de Passe <i class="help" data-tooltip="Le mot de passe doit contenir au moins 8 caractères dont au moins une lettre majuscule, une lettre minuscule, un chiffre et un caractère spécial"></i></label>
        <input type="password" name="password" id="loginPassword" minlength="8" pattern="(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[-+!*$@%_])([-+!*$@%_\w]+)" required />
        <label id="showPasswordToggle" for="showPasswordCheck">Afficher
    ...
    <?php if (!empty($errors)) : ?>
        <div id="formStatusMessage" class="messageIntoForm error">
            <p class="text">Il y a des erreurs dans le formulaire</p>
        </div>
    <?php endif; ?>
</form>
